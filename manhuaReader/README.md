...
### 智能 Tooltip 系统
阅读器采用纯 CSS `:hover` 实现的 tooltip 提示，具有以下特点：
...
- 鼠标悬停在滑块上时显示当前页码
- 拖动时实时跟随滑块位置
- 使用 CSS 变量动态定位，完全跟手无延迟

#### 技术特性
- ✅ **纯 CSS 实现**：使用 `:hover` 伪类控制显示，性能优异
- ✅ **精确定位**：relative + absolute 定位，位置准确
- ✅ **平滑动画**：淡入淡出 + 轻微位移，体验流畅
- ✅ **毛玻璃背景**：`backdrop-filter: blur(8px)` 适配主题
- ✅ **小箭头装饰**：CSS 三角形指向目标元素
- ✅ **零闪烁**：无 JavaScript 响应式延迟
