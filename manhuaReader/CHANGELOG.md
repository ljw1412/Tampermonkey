...
### ✨ 新增功能
- **阅读器设置面板**：新增设置对话框，集中管理阅读器配置
  - 主题风格设置：亮色/暗色主题切换（单选按钮样式）
  - 预载数量设置：0-5张图片可选，动态显示预载策略提示
- **持久化存储**：所有设置项使用 GM_getValue/GM_setValue 持久化保存
- **Grid 布局优化**：设置面板采用 Grid 布局，label 与选项对齐美观
- **按钮式单选框**：将传统单选框改为现代化按钮样式，交互体验更佳

### 🔧 技术改进
- **响应式状态管理**：新增 `preloadCount` 响应式变量管理预载数量
- **动态预载逻辑**：当预载数量为 0 时，preloadImages 直接返回空数组
- **智能提示文本**：根据预载数量动态显示不同的提示文案
  - 0："当前页前后不进行预载"
  - 1-5："当前页前后各预载X页"
- **CSS Grid 布局**：设置项使用 `grid-template-columns: auto 1fr` 实现左右对齐
- **样式优化**：
  - `.vmr-setting-item`：列间隔 24px，行间隔 8px
  - `.vmr-setting-hint`：跨列显示，字体 12px，行高 16px
  - `.vmr-radio`：圆角胶囊样式，选中时带边框高亮
...
- **按钮式单选框**：
  ...
  - 选中时蓝色边框 + 文字加粗
  - 过渡动画流畅自然
- **提示文本优化**：位于选项下方，灰色小字，不干扰主要操作

### 📝 代码质量
- **模板结构优化**：`.vmr-setting-hint` 作为 `.vmr-setting-options` 的兄弟节点
- **样式隔离**：所有设置相关样式使用 `.vmr-setting-` 前缀
- **可维护性提升**：设置项采用统一的 Grid 布局模式，易于扩展新设置

### 🐛 Bug 修复
- 修复设置面板中单选框值范围错误（从 1-6 修正为 0-5）
- 修复 `.vmr-setting-hint` 位置不正确的问题
- 确保预载数量为 0 时不会加载任何额外图片
